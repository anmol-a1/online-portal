...
const ExamDetails = (props) => {
    ...
    const handleSubmit = (e) => {
        ...
        const deletedtudent = async () => {
            let response = await fetch('137.184.88.170/teacher/finalizetotaltime/' + props.element.id);
            let response1 = await fetch('137.184.88.170/teacher/finalizetotalmarks/' + props.element.id);
            ...
            try {
                const was = await fetch('137.184.88.170/teacher/editexam/' + props.element.id, {
                    method: 'PUT',
                    headers: {
