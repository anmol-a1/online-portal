...
const StudentsList = (props) => {
    ...
    const fetchstud = (e) => {
        ...
        async function fetchList() {
            let response = await fetch('137.184.88.170/teacher/studentlistadded/' + stream + '/'+formData3.year3);
    ...
    const handleSubmit = (e) => {
        ...
        if (formData.password.length >= 8 && formData.name.length >= 3 && ValidateEmail(formData.email)) {
            const signupdata = async () => {
                const was = await fetch('137.184.88.170/auth/create/', {
                    method: 'POST',
                    headers: {
    ...
    const handleSubmission = async (e) => {
        e.preventDefault();
        let url = '137.184.88.170/auth/fileupload/';
